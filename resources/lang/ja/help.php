...
return [
  ...
  'featured_img_size' => '注目の画像サイズは285x190pxである必要があります',
  'brand_featured_img_size' => '注目の画像サイズは380x160pxである必要があります',
  'featured_image' => 'この画像はホームページの注目カテゴリセクションに表示されます',
  'brand_featured_image' => 'この画像は、ホームページの注目ブランドセクションに表示されます',
  ...
  'slider_description' => 'スライダーについてもう少し。説明を表示したくない場合は、空白のままにします。',
  ...
  'config_can_cancel_order_within' => 'ご注文後、この時間内にキャンセルすることができます。注文が完了するまでキャンセルできるように、空のままにしておきます。キャンセルオプションを無効にするには、0を設定します。顧客は引き続きベンダーにキャンセルを要求できます。',
  'mobile_app_slider_hits' => '比率を2：1に保つ',
  'enable_live_chat_on_platform' => '有効にすると、ベンダーは製品ページとストアページでライブチャットをオン/オフにするオプションを取得します。',
  'enable_live_chat_on_shop' => '商品ページとストアページでライブチャットを有効にします。',
  'package_dependency_not_loaded' => '依存関係に失敗しました！このパッケージは、 :dependencyモジュールに依存しています。',
  'option_dependence_module' => '依存関係に失敗しました！このオプション依存 :dependencyモジュール',
  'config_vendor_order_cancellation_fee' => 'ベンダーが注文をキャンセルした場合のキャンセル料。キャンセル料なしの場合は0に設定し、注文ごとにカスタム料金を設定するには空のままにします（キャンセルには管理者の承認が必要です）',
  'vendor_order_cancellation_fee' => '注文キャンセル料はベンダーに請求されます。',
  'disabled_when_vendor_get_paid_directly' => 'ベンダーが直接支払いを受ける場合は有効にできません！',
  'confirm_uninstall_package' => ' :packageに関連するすべてのデータは永久に失われます！これらのデータを元に戻すことはできません。',
  'verify_license_key' => '購入時にライセンスキーをメールで送信しました。見つからない場合は、詳細をサポートにご連絡ください。',
  'promotional_tagline' => 'プロモーションのキャッチフレーズはメインナビゲーションに配置されます。',
  'best_finds_under' => 'これは、ホームページの<em> Best Finds Under </em>製品カルーセル用です。システムは、この価格制限の下で最も売れているアイテムを選びます。',
  'featured_items' => 'これはホームページの<em>注目</em>セクション用です。 5〜10個のアイテムを設定することをお勧めします。',
  'featured_categories' => 'これはホームページ<EM>のカテゴリ</em>セクションです。 10~15のカテゴリを設定することをお勧めします。',
  'trending_now_categories' => 'これは、ホームページ<em>トレンドのカテゴリ</em>セクションの場合です。 2-4のカテゴリを設定することをお勧めします。',
  'featured_brands' => 'これはホームページ<em>の注文ブランド</em>セクションです。 3つのブランドを設定することをお勧めします。',
  'slider_alternative_color' => '色は、スパンタグ内のテキストに使用されます。',
  'you_can_use_span_tag' => '<span>タグを使用して、重要な単語を強調表示できます。',
  'trending_now_category_help' => '見やすくするには、最大5つのカテゴリを追加します',
  'social_auth' => 'ソーシャルログインを有効にすると、顧客のログインと登録ページにオプションが表示されます',
  'slider_text_position' => 'スライダーでコンテンツの位置を設定します。デフォルト位置右',
  'deal_of_the_day' => 'その日の取引として設定できるアイテムは1つだけです。',
  'upload_package_zip_archive' => 'パッケージファイルを含むzipアーカイブをアップロードします。ドキュメントやその他のファイルをアップロードしないでください。',
  'help_clear_cache' => '設定、画像、ルートなどのシステムキャッシュをクリアします。このアクションは、.envファイルまたは設定ファイルの変更を加えた後に必要になることがあります。すぐにあなたは少しのパフォーマンスの向上に気付くでしょうが、心配しないでください、それは最初の負荷のみのためだけです。',
  'this_will_overwrite_by_dynamic_commission' => '動的コミッションプラグインがアクティブになっているとき。これは動的値で上書きされます。',
  'transaction_fee_will_charge' => '手数料がゼロであっても取引料金が請求されます。',
  'icon_size' => 'アイコンは32x32px .p​​ng imageであるべきです',
  'icon_image' => 'このアイコン画像はカテゴリグループのアイコンとしてカテゴリグループのドロップダウンに表示されます。',
  'changes_can_take_time' => '結果に影響を与えるために変更が :timeになることがあります。',
];
