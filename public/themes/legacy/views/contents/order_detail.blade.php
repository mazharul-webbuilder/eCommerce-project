      ...
      <div class="col-md-12 nopadding">
        <table class="table" id="buyer-payment-detail-table">
          ...
          <tbody>
            ...
            @if (!$order->isPaid() && $order->paymentMethod->type == \App\Models\PaymentMethod::TYPE_MANUAL)
              <tr>
                <td colspan="6">
                  <h5>@lang('theme.payment_instruction'):</h5>
                  ...
                  <td rowspan="{{ $loop->count }}" class="order-actions">
                    ...
                    @unless($order->isCanceled())
                      ...
                      @elseif($order->canRequestCancellation())
                        ...
                        <a href="{{ route('cancellation.form', ['order' => $order, 'action' => 'cancel']) }}" class="modalAction btn btn-default btn-sm btn-block flat"><i class="fa fa-times"></i> @lang('theme.cancel_items')</a>
                      ...
                      @if ($order->isFulfilled())
                        @if ($order->canRequestReturn())
                          <a href="{{ route('cancellation.form', ['order' => $order, 'action' => 'return']) }}" class="modalAction btn btn-default btn-sm btn-block flat"><i class="fa fa-undo"></i> @lang('theme.return_items')</a>
                        @endif
