<nav class="navbar navbar-default navbar-main navbar-light navbar-top">
  <div class="container">
      ...
      @foreach ($search_category_list as $search_category_grp)
        <optgroup label="{{ $search_category_grp->name }}">
          @foreach ($search_category_grp->subGroups as $search_category)
            <option value="{{ $search_category->slug }}" @if (Request::has('insubgrp'))
    ...
    ', ' .
    Auth::guard('customer')->user()->getName() }}</span> {{ trans('theme.manage_your_account') }}
          </a>
          <ul class="dropdown-menu nav-list">
            ...
            @if (customer_has_wallet())
              <li><a href="{{ route(config('wallet.routes.wallet')) }}"><i class="fa fa-money fa-fw"></i> @lang('wallet::lang.my_wallet')</a></li>
            @endif
...
<nav class="navbar-default navbar-main navbar-light navbar-main border-bottom">
            ...
            @foreach ($all_categories as $catGroup)
              @if ($catGroup->subGroups->count())
                ...
                <li><a href="{{ route('categoryGrp.browse', $catGroup->slug) }}">
                    <i class="fa {{ $catGroup->icon ?? 'fa-cube' }} fa-fw category-icon"></i>{{ $catGroup->name }}</a>
  ...
  @if ($catGroup->images->first() && Storage::exists(optional($catGroup->backgroundImage)->path))
    <img class="nav-category-section-bg-img" src="{{ get_storage_file_url(optional($catGroup->backgroundImage)->path, 'full') }}" alt="{{ $catGroup->name }}" title="{{ $catGroup->name }}" style="margin-right: {{ in_array($catGroup->id, config('system.demo.slider_negative_margin', [])) ? '-30px' : '0' }};" />
  @endif
  ...
  <ul class="nav navbar-nav navbar-right">
    ...
    <li><a href="{{ get_page_url(\App\Models\Page::PAGE_CONTACT_US) }}" class="navbar-item-mergin-top" target="_blank">{{ trans('theme.nav.support') }}</a>
    </li>
