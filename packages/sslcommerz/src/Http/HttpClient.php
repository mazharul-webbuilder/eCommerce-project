...
namespace Incevio\Package\SslCommerz\Http;
...
class HttpClient
{
    ...
    public function __construct(Request $request)
    {
        ...
        // Set environment
        $this->sandbox = config('sslcommerz.api.sandbox', true);

        // Set default API endpoint
        $this->baseUrl = $this->sandbox ? config('sslcommerz.sandboxUrl') : config('sslcommerz.liveUrl');

        // Set Payment API URL
        $this->apiUrl = $this->baseUrl . config('sslcommerz.apiUrl.make_payment');

        // Set platorm credentials as API default
        $this->setStoreId(config('sslcommerz.api.store_id'));
        $this->setStorePassword(config('sslcommerz.api.store_password'));
    }
    ...
    {
        $trx_id = $this->request->input('tran_id');
        $amount = $this->request->input('amount');
        $currency = $this->request->input('currency');

        if (!$trx_id || !$currency || !$amount) {
        ...
        $this->post_data['store_id'] = $this->getStoreId();
        $this->post_data['store_pass'] = $this->getStorePassword();

        if ($this->SSLCOMMERZ_hash_verify($this->post_data, $this->getStorePassword())) {
            $val_id = urlencode($this->post_data['val_id']);
            $store_id = urlencode($this->getStoreId());
            $store_passwd = urlencode($this->getStorePassword());

            $url = $this->baseUrl . config('sslcommerz.apiUrl.order_validate') . "?val_id=" . $val_id . "&store_id=" . $store_id . "&store_passwd=" . $store_passwd . "&v=1&format=json";
            ...
            if ($code == 200 && !(curl_errno($handle))) {
                ...
                if ($result->status == "VALID" || $result->status == "VALIDATED") {
                    if (
                        trim($trx_id) == trim($result->tran_id) &&
                        (abs($result->amount - $result->currency_amount) < 1) &&
                        trim($currency) == trim($result->currency_type)
                    ) {
                        return true;
                    }
            ...
            throw new PaymentConfigInvalid(trans('sslcommerz::lang.config_error'));
    ...
    public function setEnv($sandbox)
    {
